...
static Instancing::AnInstance s_QuadInstances[] = {
	    {glm::vec3{glm::radians(0.0f), 1.0, 1.0}, glm::vec2{-0.5, -0.5}, glm::vec2{0.0, 0.0}, glm::vec2{1.0f, 1.0f}, 0},
	    {glm::vec3{glm::radians(0.0f), 1.0, 1.0}, glm::vec2{ 0.5, -0.5}, glm::vec2{0.0, 0.0}, glm::vec2{1.0f, 1.0f}, 0},
	    {glm::vec3{glm::radians(0.0f), 1.0, 1.0}, glm::vec2{-0.5,  0.5}, glm::vec2{0.0, 0.0}, glm::vec2{1.0f, 1.0f}, 0},
	    {glm::vec3{glm::radians(0.0f), 1.0, 1.0}, glm::vec2{ 0.5,  0.5}, glm::vec2{0.0, 0.0}, glm::vec2{1.0f, 1.0f}, 0},
	};
...
void Instancing::Instance::render (double latency) {
	...
	auto record_command_buffer = [&](VkCommandBuffer command_buffer
			...
			,VkBuffer instances_data_buffer, VkBuffer vertex_buffer, VkBuffer index_buffer
			,const uint32_t instances_count, const uint32_t indices_count
			,VkPipelineLayout pipeline_layout, VkDescriptorSet descriptor_set, VkExtent2D extent) 
	{
		...
		vkCmdBindVertexBuffers (command_buffer, 0 /*VK_VERTEX_BUFFER_BIND_ID*/, 1, &vertex_buffer, offsets);
	    vkCmdBindVertexBuffers (command_buffer, 1 /*VK_INSTANCE_BUFFER_BIND_ID*/, 1, &instances_data_buffer, offsets);
	    vkCmdBindIndexBuffer (command_buffer, index_buffer, 0, VK_INDEX_TYPE_UINT16);
	    vkCmdBindDescriptorSets(command_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS, pipeline_layout, 0, 1, &descriptor_set, 0, nullptr);   
	    vkCmdDrawIndexed (command_buffer, indices_count /*no. of indices*/, instances_count /*no. of instances*/, 0, 0, 0);
	...
	record_command_buffer(m_Context.Vk.CommandBuffers[current_frame_flight]
		...
		,m_Context.Vk.Extras.InstanceBuffer, m_Context.Vk.Extras.VertexBuffer, m_Context.Vk.Extras.IndexBuffer
		,std::size (s_QuadInstances), std::size (s_QuadIndices)
		,m_Context.Vk.PipelineLayout, m_Context.Vk.Extras.DescriptorSets[current_frame_flight]
		,m_Context.Vk.SwapchainImageExtent);

	{ // update uniform info
		Instancing::UniformBufferObject ubo {
			.model = glm::scale(glm::rotate(glm::mat4(1.0f), float(GetRenderTimestamp () * glm::radians(30.0)), glm::vec3(0.0f, 0.0f, 1.0f)), glm::vec3(3)),
			.view = glm::lookAt(glm::vec3(2.0f, -2.0f, 4.0f), glm::vec3(0.0f, -3.0f, 0.0f), glm::vec3(0.0f, 0.0f, 1.0f)),
...
void Instancing::Instance::terminateVk () {
	...
	vkDestroyBuffer (m_Context.Vk.LogicalDevice, m_Context.Vk.Extras.InstanceBuffer, nullptr);
	vkFreeMemory (m_Context.Vk.LogicalDevice, m_Context.Vk.Extras.InstanceBufferMemory, nullptr);
...
void Instancing::Instance::create_swapchain_and_related () {
	...
	{ // Create Graphics Pipeline
		...
		auto attribute_descriptions = Vertex::getAttributeDescriptions (); {
			auto more_attribute_descriptions = AnInstance::getAttributeDescriptions<3> ();
...
void Instancing::Instance::create_buffers () {
	{ // instance buffer for quads
		VkDeviceSize buffer_size = sizeof(s_QuadInstances);
		...
		memcpy (staging_buffer_ptr, s_QuadInstances, buffer_size);
		vkUnmapMemory(m_Context.Vk.LogicalDevice, staging_buffer_memory);
		// memcpy_s (buffer_ptr, alloc_info.allocationSize, s_QuadInstances, sizeof(s_QuadInstances)); // bug in microsoft stl header units

		Helper::createBuffer (m_Context.Vk.Extras.InstanceBuffer, m_Context.Vk.Extras.InstanceBufferMemory
			,m_Context.Vk.LogicalDevice, m_Context.Vk.PhysicalDevice, buffer_size
			,VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_VERTEX_BUFFER_BIT
			,VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);

		Helper::copyBuffer (m_Context.Vk.Extras.InstanceBuffer, staging_buffer
...
void Instancing::Context::struct_uitext::create_instance_buffer (VkDevice logical_device, VkPhysicalDevice physical_device) {
